...
import {
	CACHE_MISS_METRIC_SCENARIOS,
	type CacheMissMetricScenario,
} from "./scenarios";

const network = setupNetwork();

function createCacheMissGroups(scenario: CacheMissMetricScenario) {
	const { countriesPerZone, hostsPerCountry, trafficPerHost } = scenario.scale;
	return Array.from({ length: countriesPerZone }, (_, countryIndex) =>
		Array.from({ length: hostsPerCountry }, (_, hostIndex) => ({
			...
			count: trafficPerHost.count,
			avg: {
				originResponseDurationMs: trafficPerHost.avgOriginDurationMs,
			},
		})),
	).flat();
}
...
describe("cache-miss-metrics Durable Object", () => {
	it.each(
		CACHE_MISS_METRIC_SCENARIOS,
	)("$path $name", async (scenario: CacheMissMetricScenario) => {
		...
		const groups = createCacheMissGroups(scenario);
		...
		if (snapshot?.queryName !== "cache-miss-metrics") {
		...
		for (const zone of snapshot.zones) {
			expect(zone.rows).toHaveLength(
				scenario.scale.countriesPerZone * scenario.scale.hostsPerCountry,
			);
			expect(
				zone.rows.every(
